...
// Jika Tidak Upload Foto Baru
    ...
    if (strlen($temp_user) > 0) {
        move_uploaded_file($file_tmp, '../assets/images/user/' . $temp_user);

        mysqli_query($connection, "UPDATE user SET nama_user = '$nama_user', username = '$username_user', password = '$password_user', jenis_kelamin = '$jenis_kelamin', 
            email_user = '$email_user', no_telp = '$no_telp', alamat = '$alamat', role = '$role', foto_user = '$temp_user' WHERE id_user = '$id' ");
    } else {
